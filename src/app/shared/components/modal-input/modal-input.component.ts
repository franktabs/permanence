...
export const CRITERE_OBJECT: { [key in ICritere['nom']]: boolean } = {
  ...
  'REPARTI NORMALEMENT': false,
  'APPARAIT DIMANCHE': false,
  'APPARAIT SAMEDI': false,
  'PEUT APPARAITRE ENSEMBLE': false,
  'PRESENT JUSTE LA NUIT': false,
  'PRESENT JUSTE LE JOUR': false,
  // 'JAMAIS RESPONSABLE': false,
  'SAMEDI JOUR': false,
  'SAMEDI NUIT': false,
  'DIMANCHE JOUR': false,
  'DIMANCHE NUIT': false,
...
export class ModalInputComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.data.type == 'PERSONNEL') {
      ...
      if (this.data.critere_object) {
        this.criteres_object = JSON.parse(
          JSON.stringify(this.data.critere_object)
        );
        this.initial_critere_object = JSON.parse(
          JSON.stringify(this.criteres_object)
        );
      }
    }
  }
  ...
  isDisabled(): boolean {
    if (
      this.data.type == 'PERSONNEL' &&
      (!this.personnel || typeof this.personnel == 'string') &&
      this.addAllOtherPersonnel == false
    ) {
      return true;
    }
    if (this.data.type == 'CRITERE') {
      ...
      for (let key in this.criteres_object) {
        ...
        if (
          this.criteres_object[keyCritere] !=
          this.initial_critere_object[keyCritere]
        ) {
          different = true;
          break;
        }
